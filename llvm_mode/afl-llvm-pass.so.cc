...
bool AFLCoverage::runOnModule(Module &M) {
  ...
  int            fd, collisions = 0, id_cnt, data_exists = 1;
        ...
        if (bb_list == NULL) {  // very first basic block
          ...
          for (BasicBlock *Pred : predecessors(&BB)) {
            ...
            if (bb_cur != NULL) {  // predecessor has a cur_loc
              ...
              if ((bb_cur = (struct bb_id *)malloc(sizeof(struct bb_id))) ==
                  NULL)
                PFATAL("malloc");
...
/*                loop_det++;
                if (loop_det % 1000000 == 0)
                  fprintf(
                      stderr,
                      "LOOP %u: cur_loc %u, ids[cur_loc] %u <= ids_coll %u, "
                      "cnt_coll %u <= max_collisions %u\n",
                      loop_det, cur_loc, ids[cur_loc], ids_coll, cnt_coll,
                      max_collisions);*/
          ...
          if (already_exists == 0) {
            ...
            if ((bb_cur = (struct bb_id *)malloc(sizeof(struct bb_id))) == NULL)
              PFATAL("malloc");
            bb_cur->bb = new std::string(
                bb_name);  // strdup(BB.LLVMGetBasicBlockName());
            bb_cur->id = cur_loc;
            bb_cur->next = bb_list;
            bb_list = bb_cur;
            ids[cur_loc]++;
      ...
      OKF("Instrumented %u locations with %d collision(s) (%s mode, ratio "
          ...
          getenv("AFL_HARDEN")
              ...
              : ((getenv("AFL_USE_ASAN") || getenv("AFL_USE_MSAN"))
                     ? "ASAN/MSAN"
                     : "non-hardened"),
          inst_ratio, (data_exists == 1) ? " (first compilation)" : "" );
